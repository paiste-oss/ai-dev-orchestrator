        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 pt-10 px-4 text-center">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              {memories.length === 0
                ? <>Noch keine Erinnerungen.<br />Nach dem Gespräch merkt sich {buddyName} relevante Informationen.</>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-white/5 text-gray-500 sticky top-0 bg-gray-950">
                <th className="text-left px-3 py-2 font-medium w-12">Typ</th>
                <th className="text-left px-2 py-2 font-medium w-20 cursor-pointer select-none hover:text-gray-300"
                  onClick={() => setSortDir(d => d === "desc" ? "asc" : "desc")}>
                  Datum {sortDir === "desc" ? "↓" : "↑"}
                </th>
                <th className="text-left px-2 py-2 font-medium">Eintrag</th>
                <th className="w-6" />
              </tr>
            </thead>
            <tbody>
              {filtered.map((m) => (
                <tr key={m.id} className="group border-b border-white/4 hover:bg-white/4 transition-colors">
                  ...
                  <td className="px-2 py-2 align-top">
                    <button onClick={() => deleteMemory(m.id)} title="Löschen"
