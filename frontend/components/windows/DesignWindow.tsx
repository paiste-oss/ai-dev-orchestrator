        ...
        <OptionRow label="Hintergrund">
            ...
            {BG_OPTIONS.map(bg => (
              <button key={bg.v} onClick={() => update({ background: bg.v, backgroundImage: "" as UiPrefs["backgroundImage"] })}
                ...
                className={`w-7 h-7 rounded-full border-2 transition-all ${
                  prefs.background === bg.v && !prefs.backgroundImage ? "border-white scale-110" : "border-transparent opacity-50 hover:opacity-90"
                }`}
          ...
          <div className="flex gap-2 mt-1.5">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 h-8 rounded-lg border text-xs transition-all ${
            ...
            >
              {prefs.backgroundImage ? (
                <><img src={prefs.backgroundImage} alt="" className="w-4 h-4 rounded object-cover" />Bild aktiv</>
              ) : (
            ...
            {prefs.backgroundImage && (
              <button onClick={() => update({ backgroundImage: "" as UiPrefs["backgroundImage"] })}
                className="px-3 h-8 rounded-lg border border-white/8 text-xs text-gray-400 hover:text-red-400 hover:border-red-500/30 transition-all">
                Entfernen
              </button>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {WINDOW_BG_OPTIONS.map(bg => (
              <button key={bg.v} onClick={() => update({ windowBg: bg.v })}
                title={bg.v}
                className={`w-7 h-7 rounded-full border-2 transition-all ${
                  (prefs.windowBg ?? "glass") === bg.v ? "border-white scale-110" : "border-transparent opacity-50 hover:opacity-90"
                }`}
                style={{ backgroundColor: bg.hex }} />
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {FONT_COLOR_OPTIONS.map(c => (
              <button key={c.v} onClick={() => update({ fontColor: c.v })}
                title={c.l}
                className={`w-7 h-7 rounded-full border-2 transition-all ${
                  prefs.fontColor === c.v ? "border-white scale-110" : "border-transparent opacity-50 hover:opacity-90"
                }`}
