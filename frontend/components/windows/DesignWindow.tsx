...
export default function DesignWindow() {
  ...
  useEffect(() => {
    apiFetch(`${BACKEND_URL}/v1/user/preferences`)
      ...
      .then(d => { if (d) setPrefs(p => ({ ...DEFAULTS, ...p, ...d })); });
  }, []);
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {BG_OPTIONS.map(bg => (
              <button key={bg.v} onClick={() => update({ background: bg.v })}
                className={`relative h-10 rounded-lg border transition-all text-[10px] font-medium ${
